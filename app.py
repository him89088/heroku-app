...
app.config['dbconfig'] = { 'host' : 'eu-cdbr-west-03.cleardb.net',
                            'user' : 'b378b30fe54d9d',
                            'password' : '727d408b',
                            'database' : 'heroku_d298d8a8ddc1266',
                        }
...
def disclaimer() -> 'html':
    if request.method == "POST":
        return redirect(url_for(('prl_page')))
    return  render_template('disclaimer.html', title='Modelling the Effects of Pain on Human Decision Making', user=session['uid'])
...
#PRL
@app.route('/prl', methods=['GET','POST'])
def prl_page() -> 'html':
    if request.method == "POST":
        return redirect(url_for(('wcst_page')))
    return  render_template('prl.html', title='Probabilistic Reversal Learning Task', user=session['uid'])
...
@app.route('/insert_prl', methods=['GET','POST'])
def insert_prl():
        ...
        if request.method == "POST":
            ...
            for rows in req_data:
                ...
                _SQL = """insert into prl
                        ...
                        values
                        (%s, %s, %s, %s, %s, %s)"""
                cursor.execute(_SQL,(userid, choice, outcome, reactionTime, loc, choiceLoc))
